...
public class PatchOperator : MonoBehaviour
{
    ...
    [Tooltip("Just the exact Exe name of the patcher, no path or directory example - Patcher.exe")]
    ...
    public bool overwriteChangelog = true;
    ...
    void Start()
    {
        ...
        try
        {
            if (Process.GetProcesses().Any(p => p.ProcessName == fullGameExeName))
            {
                Process.GetProcessesByName(fullGameExeName).First().Kill();


                AddMessage("Killed Game process.exe");
            }
        }
        catch
        {
        ...
        {
            string adjustedPatcherExeName = fullPatcherExeName;

            if (adjustedPatcherExeName.Contains("/"))
            {
                string[] patcherNameSplit = fullPatcherExeName.Split(new string[] { "/" }, StringSplitOptions.RemoveEmptyEntries);

                if (patcherNameSplit.Length > 1)
                    adjustedPatcherExeName = patcherNameSplit[1];
            ...
            launcherExe = adjustedPatcherExeName;
            launcherNoExe = adjustedPatcherExeName.Replace(operatingSystemFileExtensions[buildOperatingSystem], "");
        ...
        string adjustedGameExe = fullGameExeName;

        if (buildOperatingSystem == OperatingSystem.Mac)
        {
            adjustedGameExe = adjustedGameExe.Replace(".app", "");


            adjustedGameExe = adjustedGameExe + @".app/Contents/MacOS/" + adjustedGameExe;

        }

        if (patcherNotInGameDirectory && !File.Exists(adjustedGameExe))
        {
        ...
        if(changelogTextExists && overwriteChangelog && File.Exists(Path.Combine(rootDirectory, "changelog.txt")))
    ...
    IEnumerator GetChangeLog()
    {
        ...
        scrollViewContent.sizeDelta = new Vector3(scrollViewContent.sizeDelta.x, changelogText.preferredHeight);
    ...
    void Update()
    {
        ...
        if (playTimerStarted)
        {
            ...
            if (Time.time > playTimerStartTime + 3 && !gameStarted)
            {
                gameStarted = true;
    ...
    private void CheckAllFiles()
    {
        ...
        for (int i = 1; i < _files.Length; i++)
        {
            ...
            if (File.Exists(Path.Combine(rootDirectory, _md5split[0])) && !_md5split[0].Contains(launcherNoExe) && !_md5split[0].Contains(launcherExe))
            {
                ...
                {
                    DateTime serverFileTime = DateTime.ParseExact(_md5split[2], "MM/dd/yyyy HH:mm:ss", CultureInfo.InvariantCulture);
    ...
    private void StartUpdateCheck()
    {
        ...
        patchingThread = new Thread(threadStart);
